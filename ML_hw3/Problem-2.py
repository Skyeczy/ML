...
class Model:
    ...
    def count_words(wordlist, data):
        ...
        for i in range(len(wordlist)):
            for email in data:
                ...
                times = content_list.count(wordlist[i])
                if times != 0:
                    word_counts[label,i] = times + int(word_counts[label,i])
        print(word_counts)
        return word_counts
    ...
    def calculate_probability(label_counts, word_counts):
        ...
        #     likelihood_probs[0,i] = word_counts[0,i]/sum(word_counts[0,:])
        #     likelihood_probs[1,i] = word_counts[1,i]/sum(word_counts[1,:])

        # print("likelihood_probs",likelihood_probs)
        ################### MAP #####################  
        ...
        likelihood_probs = np.zeros([2,np.shape(word_counts)[1]])
        sum_neg = sum(word_counts[0,:])
        sum_pos = sum(word_counts[1,:])
        for i in range(np.shape(word_counts)[1]):
            # here, J = 500(according to the lecture notes), 
            # which is a threshold set to be the maximum number of words
            # that a word can be repeated in one email. 
            likelihood_probs[0,i] = (word_counts[0,i]+1)/(sum(word_counts[0,:])+500)
            likelihood_probs[1,i] = (word_counts[1,i]+1)/(sum(word_counts[1,:])+500)
    ...
    def predict(self, x):
        ...
        for i in range(len(self.wordlist)):
            ...
            if word in x:
                time = x.count(word)
                prob_non_spam += time *  np.log(self.likelihood_probs[0,i])
                prob_spam += time *  np.log(self.likelihood_probs[1,i])
            else:
                
                prob_non_spam += np.log(1-self.likelihood_probs[0,i])
                prob_spam += np.log(1-self.likelihood_probs[1,i])
